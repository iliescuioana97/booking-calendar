...
<div class="bs-docs-example tooltip-demo" style="background-color: #FFFFFF;">
    ...
    $Service = $wpdb->get_results($wpdb->prepare("SELECT * FROM `$ServiceTable` where id > %d", null));
    foreach ($Service as $GroupName) {
        ?>
        <table class="table">
            <thead>
...
//                $ServiceStaff = $wpdb->get_results($wpdb->prepare("SELECT staff_id FROM `$StaffServiceRel` WHERE `service_id` = %s ",$GroupName->id));
                $StaffServiceTable = $wpdb->get_results($wpdb->prepare("select * from `$StaffTable` as s left join `$StaffServiceRel` as r on  s.id = r.staff_id where r.service_id = %d;", $GroupName->id));
//                var_dump($StaffServiceTable); 
                foreach ($StaffServiceTable as $Staff) {
                    ?>
                    <tr class="odd" style="border-bottom:1px;">
                        <td><em><?php echo ucwords($Staff->name); ?></em></td>
                        <td class="button-column">
                            <a rel="tooltip" href="?page=manage-staff&sid=<?php echo $Staff->id; ?>" title="<?php _e("Update", "appointzilla"); ?>"><i class="icon-pencil"></i></a> &nbsp;
                                <?php if ($Staff->id != 1) { ?>
                                <a rel="tooltip" href="?page=service&sid=<?php echo $Staff->id; ?>" onclick="return confirm('<?php echo _e("Do you want to delete this service?", "appointzilla"); ?>')" title="<?php _e("Delete", "appointzilla"); ?>" ><i class="icon-remove"></i>
                    <?php } ?>
                        </td>
                    </tr>
    <?php } ?>
...
<?php } ?>
    <!---New staff div box--->
    <div id="staffbuttonbox">
        <a class="btn btn-info" href="#" rel="tooltip" class="Create Gruop" onclick="createstaffname()"><i class="icon-plus icon-white"></i> <?php _e("Add new STAFF member", "appointzilla"); ?></a></u>
    </div>

    <div style="display:none;" id="staffnamebox">
        <form method="post">
<?php wp_nonce_field('appointment_add_staff_nonce_check', 'appointment_add_staff_nonce_check'); ?>
<?php _e("Service Category name ", "appointzilla"); ?>: <input type="text" id="staffname" name="staffname" class="inputheight" />
            <button style="margin-bottom:10px;" id="CreateStaff" type="submit" class="btn btn-small btn-success" name="CreateStaff"><i class="icon-ok icon-white"></i> <?php _e("Create Staff", "appointzilla"); ?></button>
            <button style="margin-bottom:10px;" id="CreateStaff" type="button" class="btn btn-small btn-danger" name="CancelStaff" onclick="cancelgrup();"><i class="icon-remove icon-white"></i> <?php _e("Cancel", "appointzilla"); ?></button>
        </form>
    </div>
    ...
    if (isset($_POST['CreateStaff'])) {
        ...
        global $wpdb;
        $staffname = sanitize_text_field($_POST['staffname']);
        $wpdb->query($wpdb->prepare("INSERT INTO `$StaffTable` ( `name` ) VALUES (%s);", $staffname));
...
<script type="text/javascript">
    ...
    jQuery(document).ready(function () {
        // create new group js
        jQuery('#CreateStaff').click(function () {
            jQuery('.error').hide();
            var gruopname = jQuery("input#staffname").val();
            if (gruopname == "") {
