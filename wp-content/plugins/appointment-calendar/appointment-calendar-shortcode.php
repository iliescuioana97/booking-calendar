...
//send smtp email to admin
    ...
    ?>,
                ...
                dayNamesShort: ["<?php _e("Sun", "appointzilla"); ?>", "<?php _e("Lun", "appointzilla"); ?>", "<?php _e("Ma", "appointzilla"); ?>", "<?php _e("Mie", "appointzilla"); ?>", "<?php _e("Joi", "appointzilla"); ?>", "<?php _e("Vin", "appointzilla"); ?>", "<?php _e("Sam", "appointzilla"); ?>"],
                buttonText: {
                today: "<?php _e("Azi", "appointzilla"); ?>",
                        day: "<?php _e("Zi", "appointzilla"); ?>",
                        week:"<?php _e("Saptamana", "appointzilla"); ?>",
                        month:"<?php _e("Luna", "appointzilla"); ?>"
                },
                selectable: false,
                selectHelper: false,
                ...
                events: [
                        /*
                         ...
                         jQuery("#service1").change(function() {
                         var idserviciu = jQuery("#").val();
                         jQuery.ajax({
                         method: "GET",
                         url: url,
                         data: { action: "afisare_calendar_serviciu", serviciu1: idserviciu}
...
// subtract 1 from $startdate month digit coz calendar work on month 0-11
    ...
    <?php
    $AppTable = $wpdb->prefix . "ap_appointments";
    $AppTableData = $wpdb->get_results($wpdb->prepare("select `email`,`start_time`,`end_time`,`date` from `$AppTable`"));
    foreach ($AppTableData as $Client) {
//        $time = strtotime($Client->date);
//        $myFormatForView = date("d-m-Y", $time);
        $myFormatForView = date("d-m-Y", strtotime($Client->date));
        ?>
            if ((jQuery('#clientemail').val() === '<?php echo $Client->email; ?>') && (jQuery('#appdate').val() === '<?php echo $myFormatForView; ?>'))
            {                                    
                jQuery("#clientname").after("<span class='apcal-error'><br><strong><?php _e("Already have an appointment at this date", "appointzilla"); ?></strong></span>");
                return false;
            } else{console.log(jQuery('#clientemail').val());
            ...
            <form action="" method="post" name="addnewappointment" id="addnewappointment" >
                <div class="apcal_modal-info">
                    <div class="apcal_alert apcal_alert-info">
                        ...
                        <p><strong><?php _e("Creeaza o noua rezervare", "appointzilla"); ?></strong></p>
                        <div><?php _e("Selecteaza data & serviciul", "appointzilla"); ?></div>
                    </div>
                </div>
                ...
                <div class="apcal_modal-body">
                    ...
                    <div id="secdiv" style="float:right;">
                        <strong><?php _e("Data rezervarii", "appointzilla"); ?>:</strong><br>
                        ...
                        $AllEmployees = $wpdb->get_results($wpdb->prepare("SELECT * FROM `$EmployeeTable`"), OBJECT);
                        ?>
                ...
                <form method="post" name="appointment-form2" id="appointment-form2" action="" onsubmit="return CheckValidation()">
                    ...
                    <div class="apcal_modal-info">
                        <div class="apcal_alert apcal_alert-info">
                            ...
                            <p><strong><?php _e("Creeaza o noua rezervare", "appointzilla"); ?></strong></p>
                            <div><?php _e("Alege ora & completeaza cu datele personale", "appointzilla"); ?></div>
                        </div>
                    </div>
                    ...
                    <div class="apcal_modal-body">
                        <div id="timesloatbox" class="apcal_alert apcal_alert-block" style="float:left; height:auto; width:90%;">
                            ...
                            if ($TodaysAllDayEvent) {
                                ...
                                echo "<div class='apcal_alert apcal_alert-info' align='center'>" . __("Ore disponibile pentru", "appointzilla") . " <strong>'$ServiceData->name'</strong> " . __("pe", "appointzilla") . " <strong>'" . date("d-m-Y", strtotime($AppointmentDate)) . "'</strong></div>";
                        ...
                        <?php if (!$Enable && !$TodaysAllDayEvent) { ?>
                            ...
                            <table width="100%" id="bordercssremove">
                                <tr>
                                    <td width="30%" align="left" scope="row"><strong><?php _e("Nume", "appointzilla"); ?></strong></td>
                                ...
                                <tr>
                                    <td align="left" scope="row"><strong><?php _e("Telefon", "appointzilla"); ?></strong></td>
                                    <td align="center" valign="top"><strong>:</strong></td>
                                ...
                                <tr>
                                    <td align="left" valign="middle" scope="row"><strong><?php _e("Observatii", "appointzilla"); ?></strong></td>
                                    <td align="center" valign="top"><strong>:</strong></td>
                                ...
                                <tr id="buttonbox">
                                    ...
                                    <td>&nbsp;</td>
                                    <td >
                                        <a class="apcal_btn apcal_btn-alert" id="back" onclick="Backbutton()"><i class="icon-arrow-left"></i> <?php _e("Inapoi", "appointzilla"); ?></a>
                                        <button name="booknowapp" class="apcal_btn apcal_btn-success" type="button" id="booknowapp" onclick="CheckValidation()"><i class="icon-ok icon-white"></i> <?php _e("Rezerva", "appointzilla"); ?></button>
                                    </td>
                                </tr>
                            </table>
                            <div id="loading2" style="display:none; color:#1FCB4A;"><?php _e('Creem rezervarea, va rugam asteptati...', 'appointzilla'); ?><img src="<?php echo plugins_url() . "/appointment-calendar/images/loading.gif"; ?>" /></div>
                            <style type="text/css">
